...
import { ApplicationCommandMixin, ApplicationGuildMixin, MetadataStorage, resolveIGuilds, SimpleCommandMessage, } from "./index.js";
/**
...
export class Client extends ClientJS {
            ...
            this.simpleCommands.map((cmd) => {
                ...
                const printOptions = (options, depth) => {
                    ...
                    options.forEach((option) => {
                        this.logger.log(`${tab}${option.name}: ${option.type.toLowerCase()} (${option.classRef.name}.${option.key})`);
                    });
                };
    ...
    async initGuildApplicationCommands(guildId, DCommands, options) {
        ...
        await Promise.all(ApplicationCommands.map(async (cmd) => {
            const DCommandx = DCommands.find((DCommand) => DCommand.name === cmd.name && DCommand.type === cmd.type);
        ...
        await Promise.all(commandToUpdate.map((cmd) => {
            return guild.commands.permissions
                ...
                if (!_.isEqual(permissions, commandPermissions)) {
                    if (log ?? !this.silent) {
                        this.logger.log(`${this.user?.username} >> command: ${cmd.name} >> permissions >> updating >> guild: #${guild}`);
                    }
                ...
                const commandPermissions = await cmd.instance.resolvePermissions(guild, cmd);
                if (log ?? !this.silent) {
                    this.logger.log(`${this.user?.username} >> command: ${cmd.name} >> permissions >> adding >> guild: #${guild}`);
                }
                await cmd.command.permissions.set({
                    permissions: commandPermissions,
                });
     ...
     * @returns
     */
    async executeButtonInteraction(interaction, log) {
        const botResolvedGuilds = await this.botResolvedGuilds;
        const button = this.buttonComponents.find((DButton) => DButton.isId(interaction.customId));
        const guilds = [];
        if (button) {
            guilds.push(...(await resolveIGuilds(this, button, [
                ...botResolvedGuilds,
                ...button.guilds,
            ])));
        }
        if (!button ||
            ...
            if (log ?? !this.silent) {
                `${this.user?.username ?? this.botId} >> button interaction not found, interactionId: ${interaction.id} | customId: ${interaction.customId}`;
            }
            return;
        }
     ...
     * @returns
     */
    async executeSelectMenu(interaction, log) {
        const botResolvedGuilds = await this.botResolvedGuilds;
        const menu = this.selectMenuComponents.find((DSelectMenu) => DSelectMenu.isId(interaction.customId));
        const guilds = [];
        if (menu) {
            guilds.push(...(await resolveIGuilds(this, menu, [
                ...botResolvedGuilds,
                ...menu.guilds,
            ])));
        }
     ...
     * @returns
     */
    parseCommand(prefix, message, caseSensitive = false) {
    ...
    async executeCommand(message, options) {
        ...
        const command = this.parseCommand(prefix, message, options?.caseSensitive ?? false);
