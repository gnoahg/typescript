...
class MetadataStorage {
    ...
    async build() {
        ...
        this.discordMembers.forEach((member) => {
            ...
            if (member instanceof index_js_1.DComponentButton) {
                discord.buttons.push(member);
            }
            if (member instanceof index_js_1.DComponentSelectMenu) {
        ...
        this._simpleCommands.forEach((cmd) => {
            ...
            if (cmd.prefix) {
                [...cmd.prefix].forEach((pfx) => {
                    ...
                    mapCmd.forEach((mcmd) => {
                        if (lodash_1.default.findIndex(cmds, { name: mcmd.name }) !== -1) {
                            throw Error(`Duplicate simple command name: ${mcmd.name} (of: ${mcmd.command.name})`);
                        }
                    });
                    this._simpleCommandsByPrefix.set(pfx, [...cmds, ...mapCmd].sort((a, b) => b.name.length - a.name.length));
                });
                return;
            }
